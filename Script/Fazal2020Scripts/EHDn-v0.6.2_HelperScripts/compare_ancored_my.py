...
def exclude_samples_starting_with_s(count_table):
    # Function to exclude samples starting with 'S'
    for row in count_table:
        row["sample_counts"] = {
            s: c for s, c in row["sample_counts"].items() if not s.startswith('S')
        }
    return count_table
...
def main():
    ...
    exclude_samples_starting_with_s(count_table)
    logging.info('%i regions left after secondary filtering', len(count_table))
